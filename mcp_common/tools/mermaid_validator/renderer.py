...
def _locate_mermaid_core() -> Path | None:
    ...
    1. ``MCP_COMMON_MERMAID_CORE`` env var (operator-pinned). Must pass
    ...
    env_override = os.environ.get("MCP_COMMON_MERMAID_CORE")
    if env_override:
        path = Path(env_override).resolve()
        if _is_trusted_mermaid_path(path):
            return path
        raise RuntimeError(
            f"MCP_COMMON_MERMAID_CORE={env_override} is not under a "
...
def _locate_jsdom() -> Path | None:
    ...
    Looks for the `node_modules/jsdom/` directory under the mcp-common
    ...
    Override via ``MCP_COMMON_JSDOM`` env var for CI where the
    mcp-common repo lives at a different path.
    """
    env_override = os.environ.get("MCP_COMMON_JSDOM")
    if env_override:
        path = Path(env_override).resolve()
        if path.is_file():
            return path
        raise RuntimeError(
            f"MCP_COMMON_JSDOM={env_override} does not exist or is not a file"
        )

    # Walk up from this file to find the mcp-common repo root.
...
def validate_mermaid_blocks(
    ...
    passed. The Node.js runner is `mcp_common/tools/mermaid_validator/validate_mermaid.mjs`,
    ...
    if not jsdom:
        raise RuntimeError(
            ...
            "run `npm install` in the mcp-common repo to install the "
            "wave-11 dev dep, or set MCP_COMMON_JSDOM to its absolute path"
        )
...
def print_errors(errors: list[MermaidValidationError]) -> None:
    """Pretty-print the broken blocks for `mcp-common docs check-mermaid`."""
    if not errors:
